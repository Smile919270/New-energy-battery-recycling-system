...
<div class="search">
            ...
            <Form ref="searchForm" :model="searchForm" inline :label-width="0">
                <Form-item ref="searchForm" :model="searchForm" inline :label-width="0" style="display:flex;">
                    <Form-item label="" prop="title">
                        <Input type="text" v-model="searchForm.title" placeholder="咨询标题" clearable style="width: 120px" />
                    </Form-item>
                    <Form-item label="" prop="content">
                        <Input type="text" v-model="searchForm.content" placeholder="咨询内容" clearable style="width: 120px" />
                    </Form-item>
    ...
    <Modal v-model="replyModal" title="回复留言" width="800" @on-ok="replyConsultingServiceFx">
        <Form :label-width="100" label-position="left">
            <Row :gutter="16">
                <Col span="24">
                <FormItem label="回复内容">
                    <Input v-model="replyContent" type="textarea" :rows="6" show-word-limit maxlength="240" clearable style="width:100%" />
                </FormItem>
                </Col>
                <Col span="24">
                <FormItem label="回复人">
                    <Select v-model="replyId" style="width:100%">
                        <Option v-for="(item,index) in receptionistList" :value="item.id" :key="index">{{ item.name }}</Option>
                    </Select>
                </FormItem>
                </Col>
            </Row>
...
<script>
import {
    getConsultingServiceList,
    deleteConsultingService,
    replyConsultingService,
    getReceptionistList
} from "./api.js";
...
export default {
                            ...
                            h(
                                "Button", {
                                    props: {
                                        ...
                                        icon: "ios-settings",
                                        ghost: true,
                                        disabled: params.row.replyFlag || !(that.$route.meta.permTypes && that.$route.meta.permTypes.includes("edit"))
                                    },
                                    style: {
                                        marginRight: "5px"
                                    },
                                    on: {
                                        click: () => {
                                            this.replyContent = "";
                                            this.replyId = this.receptionistList[0].id;
                                            this.replyModal = true;
                                            this.zwzSelectRow = params.row;
                                        }
                                    }
                                },
                            ),
                            ...
                            h(
                                "Button", {
                                    props: {
                                        ...
                                        icon: "md-trash",
                                        ghost: true,
                                        disabled: params.row.replyFlag || !(that.$route.meta.permTypes && that.$route.meta.permTypes.includes("delete"))
                                    },
                                    on: {
                                        click: () => {
    ...
    methods: {
        init() {
            this.getDataList();
            this.getReceptionistListFx();
        },
        replyConsultingServiceFx() {
            var that = this;
            replyConsultingService({
                dataId: that.zwzSelectRow.id,
                userId: that.replyId,
        ...
        getReceptionistListFx() {
            var that = this;
            getReceptionistList().then(res => {
                if (res.success) {
                    that.receptionistList = res.result;
                }
            })
        },
        submited() {
        ...
        getDataList() {
            this.loading = true;
            getConsultingServiceList(this.searchForm).then(res => {
                this.loading = false;
                if (res.success) {
