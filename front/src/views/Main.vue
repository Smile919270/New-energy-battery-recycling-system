...
<div class="main" :class="{'main-hide-text': shrink}">
    <div class="sidebar-menu-con menu-bar" :style="{width: shrink ? '60px' : '220px', overflow: shrink ? 'visible' : 'auto'}">
        <shrinkable-menu :shrink="shrink" @on-change="handleSubmenuChange" :theme="menuTheme" :before-push="beforePush" :open-names="openedSubmenuArr" :menu-list="visibleMenuList">
            ...
            <div class="header-middle-con">
                ...
                <div class="main-nav-menu" v-if="navType==1||navType==2">
                    <Menu mode="horizontal" :active-name="currNav" @on-select="selectNav">
                        <MenuItem v-for="(item, i) in visibleNavList.slice(0, sliceNum)" :key="i" :name="item.name">
                        <Icon :type="item.icon" v-if="navType==1" />
                        {{item.title}}
                        </MenuItem>
                        <Submenu name="sub" v-if="visibleNavList.length>sliceNum">
                            <template slot="title">更多</template>
                            <MenuItem v-for="(item, i) in visibleNavList.slice(sliceNum, visibleNavList.length)" :key="i" :name="item.name">
                            <Icon :type="item.icon" v-if="navType==1" />
                            {{item.title}}
                            </MenuItem>
                        </Submenu>
                ...
                <div class="main-nav" v-if="navType==3">
                    <Dropdown transfer @on-click="selectNav">
                        ...
                        <DropdownMenu slot="list">
                            <DropdownItem v-for="(item, i) in visibleNavList" :key="i" :name="item.name" :selected="currNav==item.name">
                                <div class="nav-item">
                    ...
                    <Row type="flex" justify="end" align="middle" class="user-dropdown-innercon">
                        <Dropdown transfer trigger="hover" @on-click="handleClickUserDropdown">
                            <a>
                                ...
                                <Avatar :src="avatarPath" style="background: #3dd598;margin-left: 10px;"></Avatar>
                            </a>
                            <DropdownMenu slot="list">
...
export default {
    ...
    computed: {
        ...
        visibleNavList() {
            // 使用 util 中统一的过滤函数，完全依赖后端权限配置
            return util.filterSystemBaseModule(this.navList);
        },
        visibleMenuList() {
            // 使用 util 中统一的过滤函数，完全依赖后端权限配置
            return util.filterSystemBaseModule(this.menuList);
        },
        pageTagsList() {
        ...
        avatarPath() {
            let path = localStorage.avatorImgPath;
            // 如果是相对路径，添加后端服务器地址
