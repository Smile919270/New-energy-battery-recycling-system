...
<div ref="scrollCon" @DOMMouseScroll="handlescroll" @mousewheel="handlescroll" class="tags-outer-scroll-con">
    ...
    <div ref="scrollBody" class="tags-inner-scroll-body" :style="{left: tagBodyLeft + 'px'}">
        <transition-group name="taglist-moving-animation">
            <Tag type="dot" v-for="(item, index) in pageTagsList" ref="tagsPageOpened" :key="item.name" :name="item.name" @on-close="closePage" @click.native="linkTo(item)" :closable="!(item.name=='home_index'||item.name=='my_home_index')" :color="item.children?(item.children[0].name==currentPageName?'primary':'default'):(item.name==currentPageName?'primary':'default')">{{ item.title }}</Tag>
        </transition-group>
    </div>
</div>
</template>
    ...
    methods: {
        ...
        handleTagsOption(type) {
            if (type == "clearAll") {
                ...
                const targetName = userInfo && Number(userInfo.type) === 1 ? "home_index" : "my_home_index";
                this.$router.push({
                    name: targetName
                });
            } else {
